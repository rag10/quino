...
class ApplicationService:
    ...
    def set_current_pose(self, pose: Pose | None) -> None:
        ...
        if pose is None:
            return
        project = self._require_project()
        completed = self._complete_pose(pose)
        target = self.get_current_pose()
        if target is None:
            # Register the supplied pose as the current one.
    ...
    def set_driver_initial_velocity(self, driver_id: str, value: float | None) -> None:
        ...
        # Validate driver exists (raises if not).
    ...
    def solve_current_pose(
        ...
        if working_pose is None:
            # Auto-create a reference pose on first solve, mirroring old behavior.
            reference = self.create_reference_pose()
            project.poses.append(reference)
            self._current_pose_id = reference.id
        ...
        if result.success and result.pose is not None:
            ...
            if current is not None:
                solved = self._complete_pose(result.pose)
    ...
    def _validate_sketch_constraint_references(
        ...
        if constraint_type in {
            ...
            if len(references) == 1 and len(entity_references or []) == 1:
                ...
                return  # valid radius form â€” skip generic checks below
